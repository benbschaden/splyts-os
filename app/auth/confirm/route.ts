...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL(`/invite/invalid?reason=${encodeURIComponent(error)}`, url.origin),
    )
  }
  ...
  if (exchangeError || !data.session || !data.user.email) {
    return NextResponse.redirect(
      new URL(`/invite/invalid?reason=${encodeURIComponent(exchangeError?.message ?? 'session_failed')}`, url.origin),
    )
  }

  // Look up the pending invite by the user's email — no token needed in URL
  const invite = await getPendingInviteByEmail(data.user.email)

  if (invite) {
    const { error: acceptError } = await acceptInviteById(invite.id, data.user.id)
    if (acceptError) {
      return NextResponse.redirect(
