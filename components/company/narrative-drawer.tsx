...
interface NarrativeDrawerProps {
  ...
  onClose: () => void
  onSaved: () => void
  editing: BrandNarrativeRow | null
}
...
export function NarrativeDrawer({ open, onClose, onSaved, editing }: NarrativeDrawerProps) {
      ...
      setForm(
        editing
          ? {
              ...
              usage_context: editing.usage_context ?? '',
  ...
  async function handleSave() {
    ...
    const url = editing ? `/api/brand-narratives/${editing.id}` : '/api/brand-narratives'
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-border px-6 py-5">
          <h2 className="text-sm font-semibold text-foreground">
            {editing ? 'Edit narrative' : 'New narrative'}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
          ...
          <div className="space-y-1.5">
            <label htmlFor="narrative-title" className="text-xs font-medium text-foreground">
            ...
            <input
              ...
              placeholder="e.g. Origin Story, Transformation Story"
          ...
          <div className="space-y-1.5">
            <label htmlFor="narrative-body" className="text-xs font-medium text-foreground">
              Narrative <span className="text-destructive">*</span>
            </label>
            <p className="text-[11px] text-muted-foreground -mt-0.5 mb-0.5">
              The core story — write it as you&apos;d tell it
            </p>
            <textarea
              id="narrative-body"
          ...
          <div className="space-y-1.5">
            <label htmlFor="narrative-usage" className="text-xs font-medium text-foreground">
              Usage context
            </label>
            <p className="text-[11px] text-muted-foreground -mt-0.5 mb-0.5">
              When to use this narrative — e.g. investor pitches, social posts about our mission
            </p>
            <textarea
              id="narrative-usage"
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-border p-4 hover:bg-muted/30 transition-colors">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  {form.include_in_ai
                    ? 'This narrative is included when generating on-brand content.'
