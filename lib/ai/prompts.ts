...
export function buildDiscoveryEntryAnalysisPrompt(params: {
  ...
  lines.push('')
  lines.push('  key_quote_1       — The single most revealing verbatim quote from the content.')
  lines.push('  key_quote_2       — Second most revealing verbatim quote. null if fewer than 2 standouts exist.')
  lines.push('  key_quote_3       — Third most revealing verbatim quote. null if fewer than 3.')
  lines.push('')
  lines.push('  RULES FOR KEY QUOTES — all three must follow these rules or be set to null:')
  lines.push('    1. Exact verbatim words from the content. Never paraphrase or reconstruct.')
  lines.push('    2. MUST be a complete, standalone statement. A stranger reading it with zero context')
  lines.push('       must fully understand what is being said. No dangling pronouns, no incomplete')
  lines.push('       sentences, no quotes that trail off or reference something only clear in context.')
  lines.push('    3. If the most revealing moment in the transcript is not a complete standalone')
  lines.push('       statement, expand the quote to include enough surrounding words so it IS complete.')
  lines.push('    4. Never cut a quote mid-thought. If it needs the next sentence to make sense, include it.')
  lines.push('    5. Prefer quotes that reveal the person\'s actual problem, motivation, or emotion —')
  lines.push('       not generic commentary or filler.')
  lines.push('')
